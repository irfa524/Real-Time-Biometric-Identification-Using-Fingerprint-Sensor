...
# -*- coding: utf-8 -*-
"""
Fingerprint Recognition Project - End-to-End Pipeline

Steps:
    1️- Dataset Augmentation using ImageDataGenerator (Keras)
    2️- Training ResNet18 model with Train/Validation Split (PyTorch)
    3️- Real-Time Fingerprint Capture via Sensor (if available)
    4️- Fingerprint Prediction from Image or Sensor Input
...
# Try importing fingerprint sensor library (optional)
try:
...
#  STEP 2: Dataset Augmentation Function
def augment_dataset(source_dir=r"C:\Users\HP\OneDrive\Documents\fingerprint_simple_knn\fingerprint_simple_knn\New Dataset",
                    ...
                    target_count=100):
    """
    Augment the dataset to balance each class (person) up to target_count images.
    Uses Keras ImageDataGenerator for realistic fingerprint variations.
    """
    ...
    for person in os.listdir(source_dir):
        ...
        # Copy original images to target folder
        ...
        print(f" Augmenting {person} → Target: {target_count} images ...")
...
def train_model(data_dir="New Dataset Augmented", epochs=5, save_path="best_model.pth", val_split=0.2):
    """
    Trains a ResNet18 model with a train/validation split.
    Args:
...
#  STEP 4: Fingerprint Capture from Sensor
def capture_from_sensor(port='COM3', baudrate=57600,
                        address=int('FFFFFFFF', 16), password=int('00000000', 16),
                        out_dir='captured_images'):
    """
    Captures fingerprint image using pyfingerprint sensor.
    Saves the captured image as BMP in output directory.
    """
...
def predict_image(model, class_names, image_path):
    """
    Predicts which class a fingerprint image belongs to.
...
def main():
    """
    Main entry point for the fingerprint recognition project.
    Provides menu-based execution for augmentation, training, and prediction.
    """
    print(" Fingerprint Recognition Project")
    ...
    if choice == "1":
        ...
        # Capture from sensor and predict
